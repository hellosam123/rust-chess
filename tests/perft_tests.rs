use rust_chess::{board::Board, movegen::MoveGenerator};

struct PerftPosition {
    fen: String,
    depth: u8,
    nodes: u32,
}

impl PerftPosition {
    fn new_perfts(fen: &str, result_list: Vec<(u8, u32)>) -> Vec<Self> {
        let mut perft_list = Vec::with_capacity(result_list.len());
        for (depth, nodes) in result_list {
            perft_list.push(PerftPosition {
                fen: String::from(fen),
                depth,
                nodes,
            });
        }

        perft_list
    }

    fn perft(board: &mut Board, depth: u8) -> u32 {
        if depth == 0 {
            return 1;
        }

        let mut nodes = 0;

        let move_list = MoveGenerator::generate_pseudo_legal_moves(board);
        for mv in move_list {
            let (is_legal, unmove) = board.make_move(mv, true);
            if !is_legal {
                board.unmake_move(mv, unmove);
                continue;
            }
            let sub_nodes = Self::perft(board, depth - 1);
            nodes += sub_nodes;

            board.unmake_move(mv, unmove);
        }

        nodes
    }

    fn expanded_perft(board: &mut Board, depth: u8) -> u32 {
        if depth == 0 {
            return 1;
        }

        let mut nodes = 0;

        let move_list = MoveGenerator::generate_pseudo_legal_moves(board);
        for mv in move_list {
            let (is_legal, unmove) = board.make_move(mv, true);
            if !is_legal {
                board.unmake_move(mv, unmove);
                continue;
            }
            let sub_nodes = Self::perft(board, depth - 1);
            nodes += sub_nodes;

            println!("{}: {}", mv, sub_nodes);

            board.unmake_move(mv, unmove);
        }

        nodes
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn perft_test_one() {
        let mut board = Board::new();
        board
            .parse_fen("r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq -")
            .unwrap();

        println!("{}", PerftPosition::expanded_perft(&mut board, 3));
    }

    #[test]
    fn perft_test_starting_pos() {
        let result_list = vec![(1, 20), (2, 400), (3, 8902), (4, 197281), (5, 4865609)];
        let perft_list = PerftPosition::new_perfts(
            "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1",
            result_list,
        );

        let mut board = Board::new();
        for perft in perft_list {
            board.parse_fen(&perft.fen).unwrap();
            let nodes = PerftPosition::perft(&mut board, perft.depth);
            assert_eq!(perft.nodes, nodes);
        }
    }

    #[test]
    fn perft_test_kiwipete() {
        let result_list = vec![(3, 97862)];
        let perft_list = PerftPosition::new_perfts(
            "r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq -",
            result_list,
        );
        let mut board = Board::new();
        for perft in perft_list {
            board.parse_fen(&perft.fen).unwrap();
            let nodes = PerftPosition::expanded_perft(&mut board, perft.depth);
            assert_eq!(perft.nodes, nodes);
        }
    }

    #[test]
    fn perft_test_all() {
        let result_list = vec![(1, 20), (2, 400), (3, 8902), (4, 197281), (5, 4865609)];
        let mut perft_list = PerftPosition::new_perfts(
            "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1",
            result_list,
        );

        let result_list = vec![(1, 48), (2, 2039), (3, 97862), (4, 4085603)];
        perft_list.append(&mut PerftPosition::new_perfts(
            "r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq -",
            result_list,
        ));

        let result_list = vec![
            (1, 14),
            (2, 191),
            (3, 2812),
            (4, 43238),
            (5, 674624),
            (6, 11030083),
        ];
        perft_list.append(&mut PerftPosition::new_perfts(
            "8/2p5/3p4/KP5r/1R3p1k/8/4P1P1/8 w - - 0 1",
            result_list,
        ));

        let result_list = vec![(1, 6), (2, 264), (3, 9467), (4, 422333), (5, 15833292)];
        perft_list.append(&mut PerftPosition::new_perfts(
            "r3k2r/Pppp1ppp/1b3nbN/nP6/BBP1P3/q4N2/Pp1P2PP/R2Q1RK1 w kq - 0 1",
            result_list,
        ));

        let result_list = vec![(1, 44), (2, 1486), (3, 62379), (4, 2103487)];
        perft_list.append(&mut PerftPosition::new_perfts(
            "rnbq1k1r/pp1Pbppp/2p5/8/2B5/8/PPP1NnPP/RNBQK2R w KQ - 1 8",
            result_list,
        ));

        let result_list = vec![(1, 46), (2, 2079), (3, 89890), (4, 3894594)];
        perft_list.append(&mut PerftPosition::new_perfts(
            "r4rk1/1pp1qppp/p1np1n2/2b1p1B1/2B1P1b1/P1NP1N2/1PP1QPPP/R4RK1 w - - 0 10",
            result_list,
        ));

        let mut board = Board::new();
        for perft in perft_list {
            board.parse_fen(&perft.fen).unwrap();
            board.print_board();
            let nodes = PerftPosition::perft(&mut board, perft.depth);
            println!(
                "fen: {}\ndepth: {}\nexpected nodes: {}\nrecieved_nodes: {}\n",
                perft.fen, perft.depth, perft.nodes, nodes
            );
            // assert_eq!(perft.nodes, nodes);
        }
    }
}

